        ...
        <div class="layui-input-inline">
            <select name="article_Course_class_1"  lay-verify="required" lay-filter="Course_class_1">
                ...
                @foreach($course_class as $v)
                    <option value="{{ $v->class_id }}" <?php if($article->article_Course_class_1 == $v->class_id){ ?>selected<?php } ?> >{{ $v->class_name }}</option>
                @endforeach
            </select>
            <select name="article_Course_class_2"  lay-filter="Course_class_2" style="display:none;" class="Course_class_2">
                <option value="0">----请选择-----</option>
                @foreach($course_class_1 as $v1)
                    <option value="{{ $v1->class_id }}" <?php if($article->article_Course_class_2 == $v1->class_id){ ?>selected<?php } ?> >{{ $v1->class_name }}</option>
                @endforeach
            </select>
        </div>
    </div>
...
<script>
    ...
    layui.use(['form', 'layedit'] ,function(){
        ...
        form.on('submit(formDemo)', function(data){ //这块要跟表单中的lay-filter="editStudent"对应
            $.ajax({
                url: "/admin/article/CourseupdateSave/"+article_id,
                type: "post",
                data: data.field,
                dataType: "json",
                success: function (data) {
